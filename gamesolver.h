...
void fprintGameState(GAMESTATE state, FILE* fptr)
{
	assert(state == WINSTATE || state == LOSSSTATE);
	if (state == WINSTATE)
	{
...
BOOL makemove(BoardState* board, uint8_t pos1, uint8_t pos2, uint32_t numspaces, FILE* fptr)
{
...
BOOL undomove(BoardState* board, uint8_t pos1, uint8_t pos2, uint32_t numspaces, FILE* fptr)
{
...
GAMESTATE solvepeggame(BoardState* board, FILE * fptr, uint32_t numspaces)
{
	GAMESTATE results;
	BOOL legalmove;
	...
	for (uint8_t currenthole = 0; currenthole < NUMHOLES; currenthole++) // for each position on the board
	{
		if (GetBit(board->board, currenthole)) // if there's a peg in that spot
		{
			for (uint8_t move = 0; move < NUMHOLES; move++) // iterate through the possible moves for that peg
			{
				...
				if ((legalmove = makemove(board, currenthole, move, numspaces, fptr))) // try to make the move
				{
					results = solvepeggame(board, fptr, numspaces + 1); // if the move was valid try playing the game from that point
					...
					legalmove = undomove(board, currenthole, move, numspaces, fptr); 
					assert(legalmove);
				}
			}
		}
